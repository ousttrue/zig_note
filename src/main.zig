...
fn getProc(_: ?*glfw.GLFWwindow, name: [:0]const u8) ?*const anyopaque {
    return glfw.glfwGetProcAddress(@as([*:0]const u8, @ptrCast(name)));
}
...
pub fn main() anyerror!void {
    ...
    io.ConfigFlags |= @intFromEnum(imgui.ImGuiConfigFlags._NavEnableKeyboard); // Enable Keyboard Controls
    //io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad;      // Enable Gamepad Controls
    io.ConfigFlags |= @intFromEnum(imgui.ImGuiConfigFlags._DockingEnable); // Enable Docking
    io.ConfigFlags |= @intFromEnum(imgui.ImGuiConfigFlags._ViewportsEnable); // Enable Multi-Viewport / Platform Windows
    ...
    if ((io.ConfigFlags & @intFromEnum(imgui.ImGuiConfigFlags._ViewportsEnable)) != 0) {
        style.WindowRounding = 0.0;
        style.Colors[@intFromEnum(imgui.ImGuiCol._WindowBg)].w = 1.0;
    }

    //
    // Setup Platform/Renderer backends
    //
    _ = imgui.ImGui_ImplGlfw_InitForOpenGL(@ptrCast(window.handle), true);
