...
pub const GizmoDragHandler = struct {
        ...
        if (hit.shape) |shape| {
            if (self.selected) |_| {
                if (shape.drag_factory) |factory| {
                    self.context = factory.*(zigla.Vec2.init(hit.cursor_pos.x, @intToFloat(f32, mouse_input.height) - hit.cursor_pos.y), shape.matrix.*, self.camera);
                    return;
                }
            }
        }
    ...
    pub fn drag(self: *Self, mouse_input: screen.MouseInput, _: i32, _: i32) void {
        if (self.context) |*context| {
            const m = context.drag(zigla.Vec2.init(@intToFloat(f32, mouse_input.x), @intToFloat(f32, mouse_input.height - mouse_input.y)));
